...
func TestDecodeObjectNull(t *testing.T) {
	...
	t.Run("sub obj should not be nil", func(t *testing.T) {
		...
		o := &ObjectNull{}
		err := UnmarshalJSONObject([]byte(`{"subobject": {},"subarray":[true]}`), o)
		require.NoError(t, err)
	...
	t.Run("sub obj and sub array should be nil", func(t *testing.T) {
		...
		o := &ObjectNull{}
		err := UnmarshalJSONObject([]byte(`{"subobject": null,"subarray": null}`), o)
		require.NoError(t, err)
	...
	t.Run(
		...
		func(t *testing.T) {
			...
			o := &ObjectNull{}
			err := UnmarshalJSONObject([]byte(`{"subobject": a`), o)
			require.Error(t, err)
			assert.IsType(t, InvalidJSONError(""), err)
		},
	)
	t.Run(
		...
		func(t *testing.T) {
			...
			o := &ObjectNull{}
			err := UnmarshalJSONObject([]byte(`{"subobject": na`), o)
			require.Error(t, err)
			assert.IsType(t, InvalidJSONError(""), err)
		},
	)
...
func assertResult(t *testing.T, v *TestObj, err error) {
	...
	assert.InDeltaf(t, -1.15657654376543, v.test5, 0, "v.test5 must be equal to 1.15")
...
func TestDecodeObjectJSONNull(t *testing.T) {
	...
	err := Unmarshal(json, v)
	require.NoError(t, err)
	assert.Equal(t, 0, v.test, "v.test must be 0 val")
}
...
func TestDecodeObjComplex(t *testing.T) {
	...
	require.Error(t, err, "err should not be as invalid type as been encountered nil")
	...
	assert.Equal(t, 0, result.Test3, "result.test3 is not expected value")
	...
	assert.True(t, result.Test4, "result.Test4 is not expected value, should be true")
}
...
func TestSkipData(t *testing.T) {
	...
	t.Run("error-invalid-json", func(t *testing.T) {
		...
		dec := NewDecoder(strings.NewReader(""))
		err := dec.skipData()
		require.Error(t, err, "err should not be nil as data is empty")
